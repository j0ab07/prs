...
function toggleMerchantStockFormAccess() {
    ...
    if (!form || !message) {
        console.error('Merchant stock form or message element not found');
        return;
    }
    ...
    if (userRole === 'Public') {
        inputs.forEach(input => input.disabled = true);
        message.textContent = 'Read-only access: You can view merchant stock but cannot modify it.';
    } else if (userRole === 'Merchant' || userRole === 'Government') {
        inputs.forEach(input => input.disabled = false);
        if (message.textContent === 'Read-only access: You can view merchant stock but cannot modify it.') {
            message.textContent = '';
        }
    } else {
        inputs.forEach(input => input.disabled = true);
        message.textContent = 'Access denied: Unknown user role.';
